...
- `setup_rustlike.bash` for RustLike.
...
```
to run the tests (sudo make check) an amount of iterations (default 20). It is possible to run the tests without sudo (make check), but this will take more time, and some tests will not be run.

The output of each run is saved to a txt file. Rename the output files in `gen-results.sh` for each new variant or run. The format is `perf_results/<variant-name>-<run-name>-res-$i.txt`. If executing several runs for the same variant, `run-name` needs to be changed, but both the `variant-name` and the amount of iterations needs to stay the same.
